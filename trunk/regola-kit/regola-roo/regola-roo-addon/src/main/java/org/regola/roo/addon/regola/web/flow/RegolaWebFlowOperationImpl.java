...
import org.regola.roo.addon.regola.web.flow.XmlTemplate.DomElementCallback;
...
@Component
@Service
public class RegolaWebFlowOperationImpl implements RegolaWebFlowOperations {
	 ...
	 @Reference private FileManager fileManager;
	    ...
	    public void createActionClass(final JavaType actionClass) {
	    	
	    	
	    	final String declaredByMetadataId = PhysicalTypeIdentifier
	                .createIdentifier(actionClass, Path.SRC_MAIN_JAVA
	        ...
	        final String actionPath = pathResolver.getFocusedCanonicalPath(
	                Path.SRC_MAIN_JAVA, actionClass);
...
//	    				  .getFocusedIdentifier(Path.ROOT, "cruisecontrol.xml");
	    		  ...
	    		  if (fileManager.exists(actionPath)) {
	    			  //LOGGER.warning("cruisecontrol.xml already exist!");
	    			  return;
	    		  }
	    		  
	    		  // Use the addon's template file
	    		  inputStream = FileUtils.getInputStream(getClass(),  "action-template.txt");
	    		  String newContents=IOUtils.toString(inputStream, "utf8");
	               
	    		  newContents = newContents.replaceAll("CHANGE_CLASS_NAME", actionClass.getSimpleTypeName());
	    		  newContents = newContents.replaceAll("CHANGE_PACKAGE", actionClass.getPackage().getFullyQualifiedPackageName());
	    		  
	    		   fileManager.createOrUpdateTextFileIfRequired(actionPath, newContents, false);
	          } catch (IOException e) {
				e.printStackTrace();
			} finally {
	              IOUtils.closeQuietly(inputStream);
	    ...
	    public void installWebFlow(final String flowName, final JavaPackage javaPackage) {
	        ...
	        copyTemplate("main.jsp", resolvedFlowPath);
	       
	        final JavaType actionClass = new JavaType(
	    			javaPackage.getFullyQualifiedPackageName() + "."  + 
	                StringUtils.capitalize(flowName) + "Action");
	    	
	        createActionClass(actionClass);
	        ...
	        new XmlTemplate(fileManager).update(resolvedFlowDefinitionPath,
	                new DomElementCallback() {
	                    public boolean doWithElement(final Document document,
	                        ...
	                        XmlUtils.findFirstElement("/flow/on-start/set", root)
	                          .setAttribute("value", 
	                        StringUtils.uncapitalize(actionClass.getSimpleTypeName())+ ".init()" );
	                        return true;
	                    }
	                });
	    ...
	    /* (non-Javadoc)
		 * @see org.regola.roo.addon.regola.web.flow.RegolaWebFlowOperations#isWebFlowInstallationPossible()
		 */
