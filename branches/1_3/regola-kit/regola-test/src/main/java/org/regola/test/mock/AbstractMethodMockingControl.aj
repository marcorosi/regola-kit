...
public abstract aspect AbstractMethodMockingControl percflow(mockStaticsTestMethod()) {
	...
	public class Expectations {
		...
		private class Call {
			...
			public boolean excludeArgumentsChecking() {
			...
			private void checkSignature(String lastSig, Object[] args) {
				...
				if (!Arrays.deepEquals(this.args, args)) {
		...
		public void dontCheckArguments() {
			Call call = calls.get(calls.size() - 1);
			if (call.excludeArgumentsChecking()) {
				throw new IllegalStateException("Arguments checking already excluded");
			}
			call.setCheckArguments(false);
		}
	}
	...
	public void dontCheckArgumentsInternal() {
		if (!recording) {
			throw new IllegalStateException("Not recording: Cannot set throwable value");
		}
		expectations.dontCheckArguments();
	}
